...
final class Debug
{
	/** @var array to set logged levels */
	...
	/** @var array of Fatal uncatched error debug backtrace list */
	public static $fatalErrorDebugBackTrace = array();
	...
	public static function _init()
	{
		...
		$tab = array('DebugHelpers', 'renderTab');
		$panel = array('DebugHelpers', 'renderPanel');
		self::addPanel(new DebugPanel('time', $tab, $panel));
		self::addPanel(new DebugPanel('memory', $tab, $panel));
		self::addPanel($tmp = new DebugPanel('errors', $tab, $panel));
		$tmp->data = & self::$errors;
	...
	{
		Debug::$fatalErrorDebugBackTrace = debug_backtrace();
		...
		if (!in_array($severity, self::$logLevels) && isset($types[$severity])) {
			return;
		}
	 ...
	 */
	public static function dump($var, $return = FALSE, $maxDepth = null)
	{
		if ($maxDepth) self::$maxDepth = $maxDepth;
