Ext.define('App.controller.tabs.Radios', {
	...
	_renderOverviewGraphIfAnyData: function () {
		...
		if (Tools.TypeOf(data) == 'Array' && data.length > 0) {
			this.truncateDataTextField(data, 'Value', 25, 'OriginalValue');
			...
			chart = Ext.create('App.view.charts.SingleColumns', {
				...
				series: [{
					...
					label: {
						field: 'Label',
						renderer: function (value, labelInstance, config, stores, index) {
					...
					tooltip: {
						...
						renderer: function (tooltip, record, item) {
							...
							if (countsDisplaying) {
								codeParts[2] += String(record.get('Count')) + ' (' + Ext.util.Format.number(record.get('Percentage'), '0.0 %') + ')';
							} else {
								codeParts[2] += Ext.util.Format.number(record.get('Percentage'), '0.0 %');
							}
